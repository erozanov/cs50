...
//my func for appending string
char* str_append(char* original, char* appended); 
...
{
    char* dest = malloc(strlen(path) + 20);
    ...
    for (int i = 0; i < n; i++)
    {
        if (strcmp(namelist[i]->d_name, "index.html") == 0) {
            strcpy(dest, path);
            return str_append(dest, "index.html");
        }else if(strcmp(namelist[i]->d_name, "index.php") == 0) {
            strcpy(dest, path);
            return str_append(dest, "index.php");
        }
    }
    return NULL;
}
...
char* str_append(char* original, char* appended) 
{
    ...
    for (int i = 0; i < appendedLen; i++) 
    {
        original[originalLen + i] = appended[i];
    }
    original[originalLen + appendedLen] = '\0';
    return original;
}
...
const char* lookup(const char* path)
{
    ...
    if (strcasecmp(ext, ".php") == 0) return "text/x-php";
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char* method = strtok((char*)line, delimiter);
    ...
    char* reqTarget = strtok(NULL, delimiter);
    ...
    char* q = strstr(reqTarget, "?");
    
    char* temp_q = NULL;
    if (q != NULL)
    {
      temp_q = malloc(strlen(q) + 1);
    ...
    int reqLen = strlen(reqTarget);
    reqTarget[reqLen] = '\0';
    strcpy(abs_path, reqTarget);
    
    char* temp_absP = malloc(strlen(reqTarget) + 1);
    temp_absP = reqTarget;
    
    char* httpVersion = strtok(NULL, delimiter);
